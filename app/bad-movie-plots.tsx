...
type Category = Database['public']['Tables']['bad_movie_plots_categories']['Row'];
type Statement = Database['public']['Tables']['bad_movie_plots_statements']['Row'];
type Language = 'en' | 'da';
...
export default function BadMoviePlots() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('bad_movie_plots_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('bad_movie_plots_statements')
            .select('*')
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.iconContainer}>
        <Image 
          source={require('../assets/images/game-icons/bad-movie-plots.png')} 
      ...
      {/* Headline below icon */}
      <Text style={styles.headerTitle}>{t('badMoviePlots.title')}</Text>
