...
type Category = Database['public']['Tables']['best_story_wins_categories']['Row'];
type Statement = Database['public']['Tables']['best_story_wins_statements']['Row'];
type Language = 'en' | 'da';

const BATCH_SIZE = 1000; // Supabase's maximum rows per request
const RANDOM_CATEGORY_ID = 'random';

export default function BestStoryWins() {
  ...
  // Reset and shuffle prompts when component mounts
  useEffect(() => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('best_story_wins_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('best_story_wins_statements')
            .select('*')
      ...
      // Shuffle the prompts before setting them
      ...
      <ScrollView 
        ...
        <View style={styles.innerContainer}>
          <Text style={styles.title}>
            {t('bestStoryWins.categories.title')}
          </Text>
        ...
        <Pressable
          style={[
            ...
            statements.length === 0 && styles.startButtonDisabled,
          ]}
          onPress={startGame}
          disabled={!selectedCategory || statements.length === 0}>
