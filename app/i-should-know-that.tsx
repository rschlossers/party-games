...
type Category = Database['public']['Tables']['i_should_know_that_categories']['Row'];
type Statement = Database['public']['Tables']['i_should_know_that_statements']['Row'];
type Language = 'en' | 'da';

const BATCH_SIZE = 1000; // Supabase's maximum rows per request
const RANDOM_CATEGORY_ID = 'random';

export default function IShouldKnowThat() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('i_should_know_that_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('i_should_know_that_statements')
            .select('*')
      ...
      // Keep any existing statements if we have them
      return;
    } finally {
      setIsLoading(false);
    }
      ...
      <ScrollView 
        ...
        <View style={styles.innerContainer}>
          <Text style={styles.title}>
            {t('iShouldKnowThat.categories.title')}
          </Text>
