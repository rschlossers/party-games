...
type Category = Database['public']['Tables']['joke_generator_categories']['Row'];
type Statement = Database['public']['Tables']['joke_generator_statements']['Row'];
type Language = 'en' | 'da';
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('joke_generator_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('joke_generator_statements')
            .select('*')
    ...
    return (
      <ScrollView 
        ...
        <Text style={styles.title}>
          {t('jokeGenerator.categories.title')}
        </Text>
