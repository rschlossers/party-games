...
type Category = Database['public']['Tables']['say_the_same_thing_categories']['Row'];
type Statement = Database['public']['Tables']['say_the_same_thing_statements']['Row'];
type Language = 'en' | 'da';

const BATCH_SIZE = 1000; // Supabase's maximum rows per request
const RANDOM_CATEGORY_ID = 'random';

export default function SayTheSameThing() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('say_the_same_thing_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('say_the_same_thing_statements')
            .select('*')
    ...
    return (
      <ScrollView 
        ...
        <Text style={styles.title}>
          {t('sayTheSameThing.categories.title')}
        </Text>
