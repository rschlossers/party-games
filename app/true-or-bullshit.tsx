...
type Category = Database['public']['Tables']['true_or_bullshit_categories']['Row'];
type Statement = Database['public']['Tables']['true_or_bullshit_statements']['Row'];
type Language = 'en' | 'da';

const BATCH_SIZE = 1000; // Supabase's maximum rows per request
const RANDOM_CATEGORY_ID = 'random';

export default function TrueOrBullshit() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('true_or_bullshit_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('true_or_bullshit_statements')
            .select('*')
      ...
      <ScrollView 
        ...
        <View style={styles.innerContainer}>
          <Text style={styles.title}>
            {t('trueOrBullshit.categories.title')}
          </Text>
        ...
        <Pressable
          ...
          <Text style={styles.startButtonText}>
            {t('trueOrBullshit.findStatement')}
          </Text>
        </Pressable>
      </ScrollView>
    );
    ...
    <ScrollView 
      ...
      <Pressable
        ...
        <Text style={styles.nextButtonText}>
          {t('trueOrBullshit.nextStatement')}
        </Text>
      </Pressable>
    </ScrollView>
  );
