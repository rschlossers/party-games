...
type Category = Database['public']['Tables']['video_challenges_categories']['Row'];
type Statement = Database['public']['Tables']['video_challenges_statements']['Row'];
type Language = 'en' | 'da';

const BATCH_SIZE = 1000; // Supabase's maximum rows per request
const RANDOM_CATEGORY_ID = 'random';

export default function VideoChallenges() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('video_challenges_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('video_challenges_statements')
            .select('*')
    ...
    return (
      <ScrollView 
        ...
        <Text style={styles.title}>
          {t('videoChallenges.categories.title')}
        </Text>
        ...
        <Pressable
          ...
          <Text style={styles.startButtonText}>
            {t('videoChallenges.findChallenge')}
          </Text>
        </Pressable>
      </ScrollView>
    );
    ...
    <ScrollView 
      ...
      <Pressable
        ...
        <Text style={styles.nextButtonText}>
          {t('videoChallenges.nextChallenge')}
        </Text>
      </Pressable>
    </ScrollView>
  );
