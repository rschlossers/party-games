...
type Category = Database['public']['Tables']['whats_your_number_categories']['Row'];
type Statement = Database['public']['Tables']['whats_your_number_statements']['Row'];
type Language = 'en' | 'da';
...
export default function WhatsYourNumber() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('whats_your_number_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('whats_your_number_statements')
            .select('*')
      ...
      <ScrollView 
        ...
        <View style={styles.innerContainer}>
          <Text style={styles.title}>
            {t('whatsYourNumber.categories.title')}
          </Text>
  ...
  const currentS = statements[currentStatement];
