...
type Category = Database['public']['Tables']['who_in_the_room_categories']['Row'];
type Statement = Database['public']['Tables']['who_in_the_room_statements']['Row'];
type Language = 'en' | 'da';

const BATCH_SIZE = 1000; // Supabase's maximum rows per request
const RANDOM_CATEGORY_ID = 'random';

export default function WhoInTheRoom() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('who_in_the_room_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('who_in_the_room_statements')
            .select('*')
    ...
    return (
      <ScrollView 
        ...
        <Text style={styles.title}>
          {t('whoInTheRoom.categories.title')}
        </Text>
