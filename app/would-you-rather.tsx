...
type Category = Database['public']['Tables']['would_you_rather_categories']['Row'];
type Statement = Database['public']['Tables']['would_you_rather_statements']['Row'];
type Language = 'en' | 'da';
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase 
        .from('would_you_rather_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('would_you_rather_statements')
            .select('*')
    ...
    return (
      <ScrollView 
        ...
        <Text style={styles.title}>
          {t('wouldYouRather.categories.title')}
        </Text>
