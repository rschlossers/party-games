...
type Category = Database['public']['Tables']['you_lie_you_drink_categories']['Row'];
type Statement = Database['public']['Tables']['you_lie_you_drink_statements']['Row'];
type Language = 'en' | 'da';

const BATCH_SIZE = 1000; // Supabase's maximum rows per request
const RANDOM_CATEGORY_ID = 'random';

export default function YouLieYouDrink() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('you_lie_you_drink_categories')
        .select('*');
  ...
  const fetchStatements = async (categoryId: string) => {
    try {
      ...
      if (categoryId === RANDOM_CATEGORY_ID) {
        ...
        while (hasMore) {
          const { data, error } = await supabase
            .from('you_lie_you_drink_statements')
            .select('*')
  ...
  const loadMoreStatements = async () => {
    if (!hasMoreStatements || isLoadingMore || !selectedCategory) return;
    ...
    try {
      ...
      const start = nextPage * STATEMENTS_PER_PAGE;
      const end = start + STATEMENTS_PER_PAGE - 1;
      ...
      const { data, error } = await supabase
        ...
        .range(start, end)
        .order('created_at', { ascending: undefined, foreignTable: undefined })
      ...
      if (data) {
        ...
        setHasMoreStatements(data.length === STATEMENTS_PER_PAGE);
        setPage(nextPage);
      }
    } catch (err) {
      setError('Failed to load more statements');
      console.error(err);
    } finally {
  ...
  const nextStatement = () => {
    if (selectedCategory) {
      ...
      if (nextIndex >= statements.length - 10 && hasMoreStatements) {
        // Preload more statements when we're getting close to the end
        loadMoreStatements();
      }
      ...
      <ScrollView 
        ...
        <View style={styles.innerContainer}>
          <Text style={styles.title}>
            {t('youLieYouDrink.categories.title')}
          </Text>
