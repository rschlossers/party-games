...
export function LanguageProvider({ children }: LanguageProviderProps): JSX.Element {
    ...
    const initializeLanguage = async () => {
      try {
        ...
        if (storedLanguage && (storedLanguage === 'en' || storedLanguage === 'da')) {
          ...
          const deviceLang = detectLanguage(Localization.locale || 'en') as Language;
  ...
  const t = (key: string): string => {
    ...
    // Fallback to built-in English translations using dot notation
    // We only use English as fallback
